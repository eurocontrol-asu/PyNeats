...
Executed by the mkdocs-gen-files plugin during build. It scans
``src/pyneats/`` for Python modules and generates a ``::: module.path``
page for each, which mkdocstrings then renders.
...
for path in sorted(src.rglob("*.py")):
    ...
    # Packages render as `<pkg>/index.md` so mkdocstrings' relative cross-refs
    # to sibling modules (e.g. `flight/#...`) resolve under the package dir.
    if is_package:
        doc_path = Path(*parts, "index.md")
        full_doc_path = Path("reference", "api", *parts, "index.md")
    else:
