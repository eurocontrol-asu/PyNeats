...
def test_pyneats_pipeline_runs_for_flight(test_inputs: Any,
                                          flight_case: Any):
    """
    ...
    if not hasattr(neats_flight, "flight_with_contrails"):
        pytest.skip("No 'flight_with_contrails' available on FlightRunner result; skipping regression sums.")

    if neats_flight.flight_with_contrails is not None:
        df_out = neats_flight.flight_with_contrails.to_dataframe()
    else:
        pytest.skip("No 'flight_with_contrails' available on FlightRunner result; skipping regression sums.")
    required_cols = ["fuel_flow", "fuel_burn", "ef"]
    missing = [c for c in required_cols if c not in df_out.columns]
    if missing:
        pytest.skip(f"Missing expected columns {missing} in output; skipping regression sums.")
    ...
    # Fetch expectations (if present)
    key = _composite_key(test_inputs["model_type"], flight_case)
    expected = test_inputs["expectations"].get(key)
    if not expected:
        pytest.skip(f"No expectations for {key}; provide with --expect to enable numeric regression.")

    rtol = float(test_inputs["rtol"])
    atol = float(test_inputs["atol"])

    # Compare using numpy allclose
    assert np.isfinite(fuel_flow_sum) and np.isfinite(fuel_burn_sum) and np.isfinite(ef_sum), "Non-finite sums"

    if "fuel_flow_sum" in expected:
        assert np.isclose(fuel_flow_sum, float(expected["fuel_flow_sum"]), rtol=rtol, atol=atol), \
            f"fuel_flow_sum {fuel_flow_sum} != expected {expected['fuel_flow_sum']} (rtol={rtol}, atol={atol})"

    if "fuel_burn_sum" in expected:
        assert np.isclose(fuel_burn_sum, float(expected["fuel_burn_sum"]), rtol=rtol, atol=atol), \
            f"fuel_burn_sum {fuel_burn_sum} != expected {expected['fuel_burn_sum']} (rtol={rtol}, atol={atol})"

    if "ef_sum" in expected:
        assert np.isclose(ef_sum, float(expected["ef_sum"]), rtol=rtol, atol=atol), \
            f"ef_sum {ef_sum} != expected {expected['ef_sum']} (rtol={rtol}, atol={atol})"