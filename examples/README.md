...
Both examples produce detailed climate impact metrics including:
...
- The cached data is required as input for both `fleet_computation_from_dataframe.py` and `fleet_computation_from_json.py`
...
The `fleet_computation_*` examples use the `FleetRunnerLargeEmitter` class, which is optimized for processing large fleets of flights with efficient parallelization and robust error handling.

Key features:
- Vectorized processing for fleet-level operations
